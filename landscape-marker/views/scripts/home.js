...
makePopup(
	document.querySelectorAll("#footer .clickable")[0],
	document.querySelector("#add_post"),
	document.querySelector("#add_post .controls .clickable")
);
makePopup(
	document.querySelectorAll("#footer .clickable")[1],
	document.querySelector("#add_place"),
	document.querySelector("#add_place .controls .clickable")
);
makePopup(
	document.querySelectorAll("#footer .clickable")[2],
	document.querySelector("#add_event"),
	document.querySelector("#add_event .controls .clickable")
);
makePopup(
	document.querySelector("#functions .parameters"),
	document.querySelector("#parameters.popup"),
	document.querySelector("#parameters .controls .clickable")
);
...
class PostSet {
	...
	render() {
		...
		for (let post of this.content.slice(
			...
			userImg.src =
				`/public/profiles/` +
				(post.user.picture ? post.user.username : "default/user.png");
...
const myPosts = new PostSet(
	document.getElementsByClassName("realContent")[0],
	"api/get_posts.php"
);
...
fetch("api/whoami.php", {
	...
	.then((raw) => raw.json())
	.then((jsoned) => {
		document.querySelector(".userImg").src = jsoned.user.picture
			? `public/profiles/${jsoned.user.username}`
			: `public/profiles/default/user.png`;
		document.querySelector(
			"#topbar > a"
		).href = `views/user.php?id=${jsoned.user.id}`;
	})
	.catch((err) => {
...
getCreateTogglable()(
	document.querySelectorAll("#navigation_bar > i"),
...
let myPlaces = new PlacesSet(
	document.getElementsByClassName("realContent2")[0],
	"api/get_places.php"
);

let myEvents = new EventsSet(
	document.getElementsByClassName("realContent3")[0],
	"api/get_events.php"
);
